	 	...
	 	Submit.setOnClickListener(new View.OnClickListener() {
            public void onClick(View v) {
            	initdata(); // Read the Textbox Values	
     			...
     			Cursor c=myDb.rawQuery("SELECT * FROM CLaneDtls WHERE LName='"+LName.getText().toString()+"' And City='"+City.getText().toString()+"'",null);
                if(c.moveToNext()){            
...
void createTable() {
    SQLiteDatabase myDb = openOrCreateDatabase(DB_NAME,Context.MODE_PRIVATE, null);
    String MySQL = "create table if not exists CLaneDtls(LID Integer not null,LName Text not null,Description TEXT not null,City TEXT not null);";
    myDb.execSQL(MySQL);
    myDb.close();
}
...
void saveData(int IDd,String LNamed,String Descrd,String Cityd) {
	...
	if (ret > 0){
		 ...
		 Toast.makeText(LaneDtlsMaster.this,"Parking Lane Details Created Successfully",Toast.LENGTH_LONG).show();
	}
	myDb.close();
}
